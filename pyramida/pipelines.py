...
class PyramidaPipeline:

    # Database setup
    conn = sqlite3.connect('pyramida.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `pyramida`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM pyramida WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO pyramida (date, title, link, content)"
